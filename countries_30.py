...
x_indexes = np.arange(len(countries_infected))    #np array from countries in order to have more plots side by side
width = 0.25
...
countries_infected.reverse()
infected_final3.reverse()

df = pd.DataFrame({'infected': infected_final3}, index=countries_infected)

ax = df.plot.barh()


plt.title("Coronavirus pandemic by country and territory in total Top 30")
plt.xlabel("People infected\n Apr 13 2020")
